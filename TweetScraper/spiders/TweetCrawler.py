...
from data_mining.TweetScraper.TweetScraper.items import Tweet, User
...
class TweetScraper(CrawlSpider):
    ...
    #query = 'from:{} since:2020-01-01'
    def __init__(self, query='# Ukraine since:2020-01-01', lang='en', crawl_user=False, top_tweet=False):
        super().__init__()
        self.query = query
        ...
        with open("TweetScraper/data/Unicorn Scraping - Links For Task 5.csv", "r") as links:
            self.users = {item[0] for item in list(csv.reader(links))[1:]}
    ...
    def start_requests(self):
        for user in tqdm(self.users):
            url = self.url % (quote(self.query.format(user)), '')
            yield http.Request(url, callback=self.parse_page, meta={"user": user})
    ...
    def parse_page(self, response):
        ...
        url = self.url % (quote(self.query.format(response.meta["user"])), min_position)
        yield http.Request(url, callback=self.parse_page, meta={"user": response.meta["user"]})
